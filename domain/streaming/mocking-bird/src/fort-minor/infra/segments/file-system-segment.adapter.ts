...
export class FileSystemSegmentAdapter implements SegmentStorePort {
  private readonly baseDir =
    process.env.FORT_MINOR_DATA_DIR ?? path.join(process.cwd(), 'data')
  private readonly outputBaseDir =
    process.env.FORT_MINOR_OUTPUT_DIR ??
    path.join(this.baseDir, 'streams')

  private readonly trackMap = new Map<string, string>([
    [DEFAULT_TRACK_ID, DEFAULT_TRACK_FILE]
  ])
